...
final class AlamofireAdapter: APISession {
    ...
    func fetchProfiles(completion: @escaping (Result<ProfileModel, any Error>) -> Void) {
        ...
        AF.request(safeUrl, method: .get).validate().responseDecodable(of: ProfileModel.self) { data in
            switch data.result {
    ...
    func fetchFeed(completion: @escaping (Result<FeedModel, any Error>) -> Void) {
        ...
        AF.request(safeUrl, method: .get).validate().responseDecodable(of: FeedModel.self) { data in
            switch data.result {
